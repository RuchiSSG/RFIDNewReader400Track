...
namespace RFIDReaderPortal.Services
{
   ...
   = new();
    ...
    = new ConcurrentDictionary<string, RfidData>();
        ...
        private void ProcessTag(string epc, DateTime timestamp)
        {
            // 🔥 PROCESS ONLY IF RACE IS ACTIVE
            if (!_raceStarted)
                return;
            ...
            // ====================================================
            else if (_eventName == "1600 Meter Running" || _eventName == "800 Meter Running")
            {
                if (rfidData.IsCompleted)
                    return;

                int maxLaps = _eventName == "1600 Meter Running" ? 5 : 3;
                TimeSpan minLapGap = TimeSpan.FromSeconds(30);
                ...
                int currentLapIndex = rfidData.LapTimes.Count - 1;
                var gap = timestamp - rfidData.LapTimes[currentLapIndex];
                ...
                if (rfidData.LapTimes.Count < maxLaps)
                {
                    ...
                    {
                        rfidData.LapTimes[currentLapIndex] = timestamp;
                    }
                ...
                if (rfidData.LapTimes.Count == maxLaps)
                {
                    ...
                    // Mark complete after stable final crossing
        ...
        private void CheckCompletion(object state)
        {
            foreach (var item in _receivedDataDict.Values)
            {
                ...
                if (item.LapTimes.Count == maxLaps)
                {
                    ...
                    if (idle >= TimeSpan.FromSeconds(5))
                    {
                        ...
                        _logger.LogInformation($"Runner {item.TagId} marked completed.");
                    }
                }
            }
        }
